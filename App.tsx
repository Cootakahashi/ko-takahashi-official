...
import { ArrowUpRight, Building2, Layers, Newspaper, ChevronDown, MapPin } from 'lucide-react';
...
const GlassCard = ({ children, className = "" }: { children: React.ReactNode; className?: string }) => (
  <div className={`backdrop-blur-xl bg-black/40 border border-white/10 shadow-[0_8px_32px_0_rgba(0,0,0,0.5)] rounded-sm ${className}`}>
    {children}
  </div>
);

const LanguageSwitcher: React.FC<{ current: LanguageCode; onChange: (lang: LanguageCode) => void }> = ({ current, onChange }) => {
...
const CategoryBadge: React.FC<{ category: string; label: string }> = ({ category, label }) => {
  ...
  const style = colors[category as keyof typeof colors] || 'text-white border-white';
  
  return (
    <span className={`text-[9px] uppercase tracking-[0.2em] px-3 py-1 rounded-sm border ${style}`}>
      {label}
    </span>
  );
};
...
const CompanyCard: React.FC<{ link: SocialLink; t: any }> = ({ link, t }) => {
  const Icon = link.iconName === 'newspaper' ? Newspaper : 
               link.iconName === 'layers' ? Layers : Building2;
  
  const companyData = link.id && t.companies[link.id] ? t.companies[link.id] : { description: '', operated_by: '' };
...
const App: React.FC = () => {
  ...
  const handleArticleClick = (articleId: string, isInternal: boolean, url: string) => {
    if (isInternal) {
  ...
  const filteredLinks = filter === 'All' ? socialLinks : socialLinks.filter(link => link.category === filter);

  // --- Sub-View Render Logic ---
  if (view === 'story' && storyData) return <><Seo currentLang={lang} pageOverride={storyData[lang]?.meta} /><StoryView onBack={() => setView('home')} initialData={storyData} /></>;
  if (view === 'schedule' && scheduleData) return <><Seo currentLang={lang} pageOverride={scheduleData.meta?.[lang]} /><ScheduleView onBack={() => setView('home')} initialData={scheduleData} /></>;
  if (view === 'articles' && articlesData) return <><Seo currentLang={lang} pageOverride={articlesData.meta?.[lang]} /><ArticlesView onBack={() => setView('home')} initialData={articlesData} onArticleClick={handleArticleClick} /></>;
  if (view === 'article_detail' && blogPostsData && articlesData) {
    const postContainer = blogPostsData.posts.find((p: any) => p.id === selectedArticleId);
    const postContent = postContainer ? (postContainer[lang] || postContainer['ja']) : null;
    const metaRaw = articlesData.articles.find((a: any) => a.id === selectedArticleId);
    return <><Seo currentLang={lang} pageOverride={{ title: postContent?.title }} /><ArticleDetailView post={postContent} meta={metaRaw} onBack={() => setView('articles')} lang={lang} /></>;
  }
      ...
      >
         ...
         <div className="flex flex-col items-end gap-6 pointer-events-auto">
           ...
           <div className="md:hidden flex gap-2">
              <button onClick={() => setView('story')} className="text-[10px] bg-black/50 border border-white/20 backdrop-blur px-3 py-1 rounded text-white">Story</button>
              <button onClick={() => setView('schedule')} className="text-[10px] bg-black/50 border border-white/20 backdrop-blur px-3 py-1 rounded text-white">Events</button>
           </div>
        ...
        >
          <GlassCard className="p-8 border-l-4 border-l-gold/80 hover:bg-black/50 transition-colors duration-500">
            ...
            <div className="flex justify-between items-center mb-8 border-b border-white/10 pb-4">
               <div>
                  <h3 className="font-serif italic text-gold text-2xl">Ko Takahashi</h3>
                  <span className="text-[10px] font-mono text-white/40 uppercase tracking-widest">Official Identity</span>
               </div>
               <div className="flex items-center gap-3 bg-white/5 px-3 py-1 rounded-full border border-white/5">
                 ...
                 <span className="text-[10px] font-mono tracking-widest text-white/70 uppercase">Tokyo, Japan</span>
               </div>
            </div>
            ...
            <div className="mt-8 flex gap-4 pt-4">
              <button onClick={() => setView('story')} className="text-xs font-mono text-gold hover:text-white transition-colors underline underline-offset-4 decoration-gold/50">
                READ MY STORY
              </button>
            </div>
          </GlassCard>
        ...
        <div className="max-w-7xl mx-auto px-6 md:px-12 py-32">
          ...
          <section className="mb-48">
            <div className="flex flex-col md:flex-row md:items-end justify-between mb-20 border-b border-white/10 pb-8">
              <h3 className="text-5xl md:text-7xl font-serif text-white tracking-tight">
                Core <span className="text-gold italic font-light">Ventures</span>
              </h3>
              <span className="hidden md:block text-xs font-mono text-white/30 mb-2 tracking-widest">01 — SELECT PROJECTS</span>
            </div>
          ...
          <section>
            <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-8">
              <div>
                 <span className="block text-xs font-mono text-gold mb-2 tracking-widest">DIGITAL PRESENCE</span>
          ...
          <footer className="mt-48 border-t border-white/5 pt-12 flex flex-col md:flex-row justify-between items-center text-xs text-white/20 font-mono">
            <div className="flex flex-col gap-2 mb-4 md:mb-0 text-center md:text-left">
              <p>&copy; {new Date().getFullYear()} Ko Takahashi. All Rights Reserved.</p>
